"""
Imagenet convert tool for versadf.
"""
import os
import time
...
class ImageNetToMR:
    r"""
    A class to transform from imagenet to versadf.
    ...
    Examples:
        >>> from mindspore.versadf import ImageNetToMR
        >>>
        >>> map_file = "/path/to/imagenet/map_file"
        >>> imagenet_dir = "/path/to/imagenet/train"
        >>> versadf_file = "/path/to/versadf/file"
        >>> imagenet_to_mr = ImageNetToMR(map_file, imagenet_dir, versadf_file, 8)
    ...
    def run(self):
        ...
        logger.info("transformed versadf schema is: {}".format(imagenet_schema_json))
    ...
    def transform(self):
        """
        Execute transformation from imagenet to versadf.

        Note:
            Please refer to the Examples of :class:`mindspore.versadf.ImageNetToMR` .
